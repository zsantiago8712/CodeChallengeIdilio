...
const InteractiveShowCard = memo(
    ({ show, onPress, width = 150, style, userScore }: InteractiveShowCardProps) => {
        ...
        return (
            <Animated.View
                ...
                className="mb-4 mr-4">
                <TouchableOpacity
                    style={[{ width }, style]} // Combinar estilos personalizados con el ancho predeterminado
                    activeOpacity={0.8}
                    onPress={() => onPress(show.id)}
                    onPressIn={handlePressIn}
                    onPressOut={handlePressOut}
                    className="rounded-3xl border border-white/10 bg-white/5 p-2.5">
                    <Box className="relative overflow-hidden rounded-2xl">
                        ...
                        {isPressed && (
                            <Box className="border-3 absolute -inset-0.5 rounded-3xl border-violet-500/80">
                    ...
                    <Box className="p-4 pb-3">
                        ...
                        <Box className="mb-2 flex-row items-center">
                            ...
                            {userScore && (
                                <Text className="ml-2 text-xs font-medium text-yellow-400">
                                    {userScore} ★
                                </Text>
                            )}
                        </Box>
